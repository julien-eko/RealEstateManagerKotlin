...
class PropertyContentProviderTest {
    ...
    fun getPropertyWhenPropertyInserted() {
        ...
        Assert.assertEquals(cursor.count, 0)
    ...
    fun insertAndGetProperty() {
        val propertyUri =
            contentResolver.insert(PropertyContentProvider.CONTENT_URI, generateProperty())
        ...
        Assert.assertEquals(cursor.getString(cursor.getColumnIndexOrThrow("type")), "Maison")
        Assert.assertEquals(
